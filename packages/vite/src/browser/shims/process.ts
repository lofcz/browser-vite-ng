...
export function createProcess(options?: {
  ...
  const env: ProcessEnv = {
    ...
    HOME: '/',
    // Signals to the fork's Node modules that we're in the browser build, so
    // their BROWSER VITE patches (CLIENT_ENTRY/ENV_ENTRY/etc.) take the
    // virtual path branch instead of on-disk resolution.
    VITE_BROWSER: '1',
    ...options?.env,
  };
