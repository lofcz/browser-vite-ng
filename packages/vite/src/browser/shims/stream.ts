...
class BufferPolyfill extends Uint8Array {
  ...
  static from(
    ...
    encodingOrMapfn?: string | number | ((v: unknown, k: number) => number),
    thisArgOrLength?: unknown
  ): BufferPolyfill {
    ...
    if (typeof encodingOrMapfn === 'function') {
      ...
      const mapped = Array.from(arrayLike, encodingOrMapfn as (v: number, k: number) => number, thisArgOrLength);
    ...
    // Buffer.from(arrayBuffer, byteOffset, length): a VIEW over the same
    // memory (not a copy). es-module-lexer relies on this to write source
    // directly into its WASM linear memory
    // (Buffer.from(E.memory.buffer, offset, len).write(src, "utf16le")).
    if (data instanceof ArrayBuffer) {
      const byteOffset = typeof encodingOrMapfn === 'number' ? encodingOrMapfn : 0;
      const length = typeof thisArgOrLength === 'number' ? thisArgOrLength : undefined;
      return BufferPolyfill._view(data, byteOffset, length);
    }
    ...
    return new BufferPolyfill(data as ArrayLike<number>);
  }

  // Create a BufferPolyfill that VIEWS an existing ArrayBuffer region without
  // copying. The BufferPolyfill constructor copies its input, so we build the
  // view via a raw Uint8Array then fix up its prototype chain.
  private static _view(buffer: ArrayBuffer, byteOffset: number, length?: number): BufferPolyfill {
    const raw =
      length === undefined
        ? new Uint8Array(buffer, byteOffset)
        : new Uint8Array(buffer, byteOffset, length);
    Object.setPrototypeOf(raw, BufferPolyfill.prototype);
    return raw as BufferPolyfill;
  }
  ...
  write(string: string, offset?: number | string, encoding?: string): number {
    // Support write(str, encoding), write(str, offset), write(str, offset, encoding)
    let off = 0;
    let enc: string = (typeof encoding === 'string' ? encoding : 'utf8').toLowerCase();
    if (typeof offset === 'number') off = offset;
    else if (typeof offset === 'string') enc = offset.toLowerCase();

    if (enc === 'utf16le' || enc === 'utf-16le' || enc === 'ucs2' || enc === 'ucs-2') {
      // Node writes utf16le: 2 bytes per code unit, little-endian. This is the
      // exact layout es-module-lexer's WASM parser reads its source from.
      const len = Math.min(string.length, (this.length - off) >> 1);
      for (let i = 0; i < len; i++) {
        const code = string.charCodeAt(i);
        this[off + i * 2] = code & 0xff;
        this[off + i * 2 + 1] = code >>> 8;
      }
      return len * 2;
    }

    if (enc === 'latin1' || enc === 'binary' || enc === 'ascii') {
      const len = Math.min(string.length, this.length - off);
      for (let i = 0; i < len; i++) this[off + i] = string.charCodeAt(i) & 0xff;
      return len;
    }

    const bytes = _encoder.encode(string);
    this.set(bytes.subarray(0, Math.max(0, this.length - off)), off);
    return Math.min(bytes.length, Math.max(0, this.length - off));
  }
